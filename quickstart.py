...
GMAIL_API_FILE      = 'gmail-api-python.json'
...
def get_credentials():
    ...
    credential_dir = os.path.join(local_dir, '.credentials')
    if not os.path.exists(credential_dir):
        os.makedirs(credential_dir)
    credential_path = os.path.join(credential_dir, GMAIL_API_FILE)
    client_path = os.path.join(credential_dir, CLIENT_SECRET_FILE)
...
def main():
    ...
    for message in messages:
        ...
        GetAttachments(service, 'me', message['id'], 'gpei_files/')
    ...
    # zip gpei_files folder
    filename = time.strftime("%Y%m%d-%H%M%S")
    zipf = zipfile.ZipFile(
        'gpei_files_{0}.zip'.format(filename), 
        'w', zipfile.ZIP_DEFLATED)
    Zipdir('gpei_files/', zipf)
